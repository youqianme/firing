...
export function Sidebar() {
  ...
  const menuItems = [
    ...
    { href: '/liabilities', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    ), label: '负债' },
    { href: '/transactions', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <>
      {/* 左侧菜单 */}
      <aside className={`hidden md:flex flex-col bg-white border-r border-slate-200 fixed top-0 left-0 bottom-0 z-10 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {menuItems.map((item) => (
            <a 
              key={item.href} 
              href={item.href} 
              className={`flex items-center px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 rounded-md ${isCollapsed ? 'justify-center' : ''}`}
            >
      ...
      {/* 主内容区域左边距 */}
      <div className={`hidden md:block transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}></div>
    </>
  );
}
