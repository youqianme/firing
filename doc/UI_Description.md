...
- 时间范围选择器：
  - 位置：标题右侧
  - 选项：6个月、1年、2年、3年、5年、10年
  - 样式：胶囊切换（`p-1 bg-slate-100 rounded-lg`）
  - 选中态：`bg-white shadow-sm text-slate-900`
  - 未选中态：`text-slate-500 hover:text-slate-900`
- 图例：
  - 资产（绿色圆点）
  - 负债（红色圆点）
  - 净资产（蓝色圆点）
  - FIRE进度（橙色圆点，仅配置了FIRE目标时显示）
- 图表：
  - 资产面积图：绿色（#10B981），显示在0轴上方
  - 负债面积图：红色（#F43F5E），显示在0轴下方（负值堆叠）
  - 净资产折线：蓝色（#3B82F6），显示资产减负债的差额
  - FIRE进度线：橙色（#F97316）虚线，右侧Y轴显示百分比
  - X轴：显示"YYYY年MM月"格式
  - 左Y轴：以"万"为单位缩写显示金额
  - 右Y轴：显示FIRE进度百分比（0-100%）
  - Tooltip：圆角白底、轻阴影；显示年月、资产负债净资产数值、FIRE进度百分比
- 数据计算逻辑：
  - 按月计算，显示每月月底的累计值
  - 基于资产/负债的startDate或createdAt计算
  - 创建之前的时间点显示为0，不进行估算
  - FIRE进度基于净资产计算：（计入FIRE的资产 - 负债）/ FIRE目标 × 100%
- 空状态：`暂无趋势数据，请添加资产或负债`
